...
(and so on)
```
In order to annotate methylation outlier against variants, you must first run the small variant, structural variant, and repeat outlier workflows. Then, you must add the paths to the output files to the `variants_for_methbat.tsv` file. 
Note the following:
