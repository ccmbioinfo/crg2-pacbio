...
def process_allele(input_filepath):
    ...
    alleles_df = pd.read_csv(
        input_filepath, sep=" ", header=None, names=["TR_ID", "SAMPLE_ID", "TR_ALLELES"]
    )

    # Creating columns for allele_length
    alleles_df[["TR_ALLELE1", "TR_ALLELE2"]] = alleles_df["TR_ALLELES"].str.split(
        ",", expand=True
    )
    ...
    for trid, sample_repeats in alleles_df.groupby("TR_ID"):
        min_allele = min(
            min(sample_repeats["TR_ALLELE1_LEN"]), min(sample_repeats["TR_ALLELE2_LEN"])
        )
        max_allele = max(
            max(sample_repeats["TR_ALLELE1_LEN"]), max(sample_repeats["TR_ALLELE2_LEN"])
        )
        min_samples = ";".join(
            sample_repeats.loc[
                (sample_repeats["TR_ALLELE1_LEN"] == min_allele)
                | (sample_repeats["TR_ALLELE2_LEN"] == min_allele),
                "SAMPLE_ID",
            ]
        )
        max_samples = ";".join(
            sample_repeats.loc[
                (sample_repeats["TR_ALLELE1_LEN"] == max_allele)
                | (sample_repeats["TR_ALLELE2_LEN"] == max_allele),
                "SAMPLE_ID",
            ]
        )
        trid_list.append(
...
def input_output_handling():
    ...
    parser = argparse.ArgumentParser(
        description="Process repeat allele data and save to a g-zipped TSV file."
    )
    parser.add_argument(
        "input_filepath",
