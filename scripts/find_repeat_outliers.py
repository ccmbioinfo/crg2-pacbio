...
def split_alleles(alleles: pd.Series) -> tuple:
    ...
    short_index, long_index = allele_lens.index(min(allele_lens)), allele_lens.index(max(allele_lens))
    ...
    return short_allele, short_allele_len, short_index, long_allele, long_allele_len, long_index


def split_meth_mp(meth_mp: pd.Series, short_index: pd.Series, long_index: pd.Series) -> tuple:
    """
...
def main(cases, dist, output_file):
    ...
    cases["short_allele"], cases["short_allele_len"], cases["short_allele_index"], cases["long_allele"], cases["long_allele_len"], cases["long_allele_index"] = zip(
        *cases.apply(lambda x: split_alleles(x.alleles), axis=1)
    )

    cases["short_allele_meth"], cases["long_allele_meth"] = zip(
        *cases.apply(lambda x: split_meth_mp(x.avg_methylation, x.short_allele_index, x.long_allele_index), axis=1)
    )

    cases["short_allele_MP"], cases["long_allele_MP"] = zip(
        *cases.apply(lambda x: split_meth_mp(x.motif_purity, x.short_allele_index, x.long_allele_index), axis=1)
    )
    ...
    # methylation
    cases_meth = pd.melt(cases, id_vars=['trid', 'sample'], value_vars=['short_allele_meth', 'long_allele_meth'], var_name="allele_am_type", value_name="AM")
    cases_meth = cases_meth.drop(columns=["trid", "sample"])

    # motif purity
    cases_mp = pd.melt(cases, id_vars=['trid', 'sample'], value_vars=['short_allele_MP', 'long_allele_MP'], var_name="allele_mp_type", value_name="MP")
