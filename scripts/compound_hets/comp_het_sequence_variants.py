...
def main():
    ...
    high_impact["GT_qual_max"] = high_impact.apply(
        lambda x: max([x[col] for col in qual_cols]), axis=1
    )
    # drop unnecessary columns
    high_impact = high_impact.drop(
        [
            "Old_multiallelic",
            "SpliceAI_score_parsed",
